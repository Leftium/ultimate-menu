export default config = '''
Businesses:
    🔒 Leftium @business(335226930254766)
        Ad Account 01 @account(626486004739350)

    🔒 Amazing Health Science @business(3992912430736092)
        AHS01 @account(2797337333703347)
        AHS02 @account(1144959642572168)
        AHS03 @account(302053170773369)

    John Murphy 1 @business(413115236387462)
        JM01 @account(365373097889567)

    🔒 John Murphy 2 @business(148595963673002)

Links:
    Ads Manager:
        Ads Manager (Campaigns) @url(https://business.facebook.com/adsmanager/manage/campaigns?act=:AID&business_id=:BID)
        Ads Manager (Ad Sets) @url(https://business.facebook.com/adsmanager/manage/adsets?act=:AID&business_id=:BID)
        Ads Manager (Ads) @url(https://business.facebook.com/adsmanager/manage/ads?act=:AID&business_id=:BID)

    Community:
        Workplace @url(https://ultimatefreedom667.workplace.com/)
        Inner Circle @url(https://innercircle.ultimatefreedom.com/)
        Healthy Commissions @url(https://members.healthycommissions.com/)

    Manage Businesses:
        Select Business @url(https://business.facebook.com/select)
        Create Business @url(https://business.facebook.com/create)

    Manage a Business:
        Business Info @url(https://business.facebook.com/settings/info?business_id=:BID)
        Business Verification Inbox @url(https://business.facebook.com/settings/support_inbox?business_id=:BID&global_scope_id=:BID)
        Pixels @url(https://business.facebook.com/settings/pixels/626797161345438?business_id=:BID)

    Manage Ad Account:
        Account Overview @url(https://business.facebook.com/adsmanager/manage/accounts?act=:AID&business_id=:BID)
        Account Quality @url(https://business.facebook.com/accountquality/:BID/:AID/)
        Payment Settings @url(https://business.facebook.com/ads/manager/account_settings/account_billing/?act=:AID&pid=p1&business_id=:BID&page=account_settings&tab=account_billing_settings)
        Invoices @url(https://business.facebook.com/ads/manager/billing/transactions/?act=:AID&pid=p1&business_id=:BID&global_scope_id=:BID&page=billing&tab=transactions)

    Images:
        Unsplash @url(https://unsplash.com)
        PixaBay @url(https://pixabay.com/)
        Pexels @url(https://www.pexels.com/)
        Google Images @url(https://images.google.com/)
        Yandex Images @url(https://yandex.com/images/)
        StockUnlimited @url(https://www.stockunlimited.com/)
        YayImages @url(https://yayimages.com/)

    Appeals:
        Manual ad appeal @url(https://business.facebook.com/help/contact/1582364792025146)
        BM appeal @url(https://www.facebook.com/help/contact/2166173276743732)
        Ads in review 24+ hour review request @url(https://www.facebook.com/help/contact/186912391909649)
        Disabled ad account @url(https://www.facebook.com/help/contact/2026068680760273)
        Automatic Payments & Ads Manager @url(https://business.facebook.com/help/contact/391647094929792)
        Ads re-enablement for a disabled ad account @url(https://business.facebook.com/help/contact/1431129123688414)
        Payments Support @url(https://business.facebook.com/help/contact/356555978365479)

    ClickBank:
        ClickBank @url(https://leftium.accounts.clickbank.com/account/mainMenu.htm)
        MarketPlace @url(https://accounts.clickbank.com/marketplace.htm)

    Misc:
        Inbox @url(https://business.facebook.com/support/)
        FB Ads Status @url(https://status.fb.com/ads)
        FB Blueprint @url(https://www.facebookblueprint.com/)
        World Time Buddy (EST CST PST) @url(http://www.worldtimebuddy.com/?lid=1835848,5128581,5037649,5391959&h=1835848)

'''
